...
- **`TurtleBotController/turtlebot.py: TurtleBotReal`** — the real robot. Spins an internal `rclpy` node (`_TurtleBotRosNode`) in a background daemon thread that subscribes to `/scan` (LaserScan) and publishes `Twist`/`TwistStamped` on `/cmd_vel`. `get_lidar_scan()` delegates to `TurtleBotController/lidar_processing.py: process_scan` (pure numpy, unit-testable without ROS — see `tests/test_lidar_hygiene.py`): it sanitizes invalid rays (NaN/inf/`0.0`/chassis reflections below `lidar_min_valid`) to `lidar_max_range` **before** anything takes a `min()`, and resamples **by angle** using the message's `angle_min`/`angle_increment` so that output index 0 is always the robot's front — the physical sensor's zero angle points to the robot's right, so the front sits at laser angle +90° (`lidar_front_angle_deg` in config; simulator-side code assumes index 0 = straight ahead). `TurtleBotReal.scan_age()` reports seconds since the last `/scan` message (`inf` if none yet); `run_real_autonomous.py` stops the robot whenever it exceeds `scan_stale_after` (0.3s) — driving on a frozen scan is driving blind. `TurtleBotMock.scan_age()` exists for interface parity and always returns 0.0. `get_vision_detections()` delegates to `TurtleBotController/vpu_vision.py: VpuYoloDetector`, which runs the `yolonanov2` model (4 classes: `left`/`right`/`stop`/`finish`, as a `.blob`) directly on the OAK-D's onboard VPU (Myriad X) via `depthai` — the `ColorCamera` and `YoloDetectionNetwork` run entirely on-device, so no image ever crosses into ROS or the Raspberry Pi's CPU; only resolved detections (class + confidence + bbox) come back over USB. Distance is still estimated from bounding-box width via the same pinhole-camera approximation as before (assumes ~20cm real-world sign width). See `DEPLOY_ROBOT_VPU.md` for the end-to-end deployment/run guide and `vpu_deployment/` for how the `.blob` was produced and validated standalone.
