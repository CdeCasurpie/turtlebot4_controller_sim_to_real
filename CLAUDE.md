...
There is no build step and no unit test suite — this is a script-based project. `test_*.py` files at the root are interactive pygame demos / manual test harnesses, not automated tests (there's nothing to run under pytest).
...
Because both classes expose the same interface, the navigation state machine is written once conceptually and duplicated as near-identical copies in `run_real_autonomous.py` (real) and `test_autonomous_controller.py` (sim, embedded in the pygame loop). **When tuning avoidance/navigation behavior, both copies need to be updated together** — they intentionally share the same state names, thresholds, and the `buscar_camino_libre()` helper (see below), which is duplicated verbatim in both files.

### Navigation state machine

Both `run_real_autonomous.py` and `test_autonomous_controller.py` (and the older, simpler `test_controller.py`) run a state machine over states like `EXPLORANDO` (cruising), `BUSCANDO_IZQ`/`BUSCANDO_DER` (a sign was seen, looking for room to turn), `GIRANDO_IZQ`/`GIRANDO_DER` (executing a fixed ~80° turn), `DETENIDO` (stopped at a `stop` sign, resumes after 3s), `FINALIZADO` (terminal state entered on a `finish` sign — stays stopped permanently; the real-robot script `break`s out of its main loop, the simulator just freezes the state so pause/rewind still works), and `EVASION_EMERGENCIA` (reactive collision avoidance overriding everything else, triggered when the nearest LiDAR point is inside a safety margin).
